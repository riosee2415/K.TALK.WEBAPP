...
import { dateArr, monthArr, yearArr } from "../selectArr";
...
const Application = () => {
  ...
  return (
    <WholeWrapper bgColor={Theme.subTheme9_C} padding={`80px 0`}>
      <RsWrapper>
        <Text fontSize={width < 700 ? `20px` : `28px`} fontWeight={`bold`}>
          Application Form
        </Text>
        <Text fontSize={width < 700 ? `16px` : `18px`} margin={`10px 0 30px`}>
          for K-talk Live regular paid Korean lessons
        </Text>
        {width < 700 ? (
          <>
            <Text fontSize={`14px`}>·Please complete and submit this form</Text>
            <Text fontSize={`14px`}>
        ...
        <CustomForm onFinish={submissionHandler} form={form} scrollToFirstError>
          ...
          <Wrapper al={`flex-start`}>
            ...
            <Wrapper dr={`row`} ju={`flex-start`}>
              <Form.Item name="nationality" rules={[{ required: true }]}>
                <CustomSelect margin={`0 10px 0 0`} placeholder="---select---">
                  {country &&
                ...
                <Form.Item name="phoneNumber" rules={[{ required: true }]}>
                  ...
                  >
                    {total &&
                      total.map((data, idx) => {
                        return (
                          <Select.Option key={idx} value={JSON.stringify(data)}>
            ...
            {timeArr &&
              timeArr.map((data, idx) => {
                return (
                  <Wrapper
                    key={idx}
                    width={width < 700 ? `calc(100% / 2)` : `calc(100% / 3)`}
                    fontSize={width < 700 ? `16px` : `18px`}
                    al={`flex-start`}
                  >
                    <CustomCheckBox2
          ...
          <Wrapper color={Theme.red_C} fontSize={width < 700 ? `14px` : `18px`}>
          ...
          <Wrapper>
            ...
            <CustomCheckBox2 checked={agreeCheck} onChange={agreeCheckToggle}>
              Yes, I agree
            </CustomCheckBox2>
          </Wrapper>
          ...
          <Wrapper al={`flex-start`}>
            ...
            >
              {width < 700 ? (
                <>
                  <Text>Please indicate your current city of residence</Text>
