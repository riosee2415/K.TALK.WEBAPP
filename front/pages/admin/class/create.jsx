...
const FormItem = styled(Form.Item)`
  width: ${(props) => props.width || `calc(100% - 90px)`};
      ...
      <AdminContent>
        ...
        >
          <GuideUl>
            ...
            <GuideLi color={Theme.red_C} margin={`0 0 5px`} isImpo>
              강의 번호는 xx-xxx 형식으로 들어가야 하며 x는 숫자로 입력되어야
              합니다.
            </GuideLi>
            <GuideLi color={Theme.red_C} margin={`0 0 5px`} isImpo>
              강의번호의 형식이 다를 시 클래스 관리 페이지에서 강의 리스트를
              정렬할 때 문제가 있을 수 있습니다.
            </GuideLi>
          </GuideUl>
        </Wrapper>
        <FormTag form={form} ref={formRef} onFinish={onSubmit}>
          ...
          <Wrapper dr={`row`} margin={`0 0 20px`}>
            ...
            <FormItem
              rules={[
                { required: true, message: "일주일별 횟수를 입력해주세요." },
              ]}
              name={`cnt`}
              {...inputCnt}
          ...
          {dayArr && dayArr.length !== 0 && (
            <Wrapper dr={`row`} ju={`flex-start`} margin={`0 0 20px`}>
              <Text width={`90px`}>수업 시간</Text>
              <Wrapper dr={`row`} ju={`flex-start`} width={`calc(100% - 90px)`}>
                {dayArr.map((data, idx) => {
                  return (
                    <FormItem
                      width={`auto`}
